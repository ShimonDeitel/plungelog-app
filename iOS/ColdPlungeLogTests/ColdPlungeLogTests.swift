import XCTest
@testable import ColdPlungeLog

@MainActor
final class ColdPlungeLogTests: XCTestCase {
    ...
    func testAddEntryIncreasesCount() {
        ...
        store.add(PlungeEntry(note: "test"))
    ...
    func testCanAddBlockedAtLimitWhenNotPro() {
        let store = Store()
        store.entries = (0..<Store.freeTierLimit).map { _ in PlungeEntry() }
        XCTAssertFalse(store.canAdd(isPro: false))
    }

    func testCanAddAlwaysAllowedWhenPro() {
        let store = Store()
        store.entries = (0..<(Store.freeTierLimit + 5)).map { _ in PlungeEntry() }
        XCTAssertTrue(store.canAdd(isPro: true))
    }

    func testDeleteEntryRemovesIt() {
        let store = Store()
        let entry = PlungeEntry(note: "to delete")
        store.add(entry)
        store.delete(entry)
    ...
    func testUpdateEntryPersistsChange() {
        let store = Store()
        var entry = PlungeEntry(note: "original")
        store.add(entry)
        entry.note = "updated"
        store.update(entry)
