...
struct ContentView: View {
    ...
    @State private var editingEntry: PlungeEntry?
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle("Cold Plunge Log")
            .toolbar {
    ...
    @ViewBuilder
    private func entryRow(_ entry: PlungeEntry) -> some View {
        HStack {
            VStack(alignment: .leading, spacing: 4) {
                Text(entry.tag.isEmpty ? "Plunge" : entry.tag)
...
struct EntryEditorView: View {
    ...
    let existing: PlungeEntry?
    let onSave: (PlungeEntry) -> Void

    init(entry: PlungeEntry?, onSave: @escaping (PlungeEntry) -> Void) {
        self.existing = entry
        self.onSave = onSave
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle(existing == nil ? "Add Plunge" : "Edit Plunge")
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        var entry = existing ?? PlungeEntry()
                        entry.tag = tag
                        entry.note = note
